...
description: |
  ...
  五阶音景系统（Web Audio API 合成，432Hz 自然调谐，五层独立音量控制）。
...
- **五行八卦**：可选光晕 + Tooltip显示
- **五阶音景**：Web Audio API 纯代码合成，呼吸层（粉色噪声）+ 建构/流转/消散/渐隐四阶段音景，432Hz 自然调谐，默认静音
- **动画录制**：桌面端优先 MP4（H.264），移动端降级 WebM（VP9），支持音频流合并
...
5. **录制功能**需通过 HTTP 服务器访问（`python -m http.server 8080`），不支持 `file://` 协议。桌面端输出 MP4（H.264），移动端输出 WebM（VP9）。录制前开启音频可获得带声音的视频
6. **五阶音景**默认静音，用户点击右下角喇叭图标开启。三态切换：🔇静音 → 🔈呼吸+音景 → 🔊仅音景
