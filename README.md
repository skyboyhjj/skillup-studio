...
| 知识树追踪引擎 | 预览版 | 基于五行理论的 AI 知识领域动态追踪，月度论文采集、结构诊断、趋势分析与漂移可视化 |
| 论文采集器 | 预览版 | arXiv 自动化月度采集，覆盖 11 个 AI 子领域；100 篇样本可筛选/搜索/导出 |
| 五行道境引擎 | V1.5 | 同态映射验证 + 壳核审计 + 种子培育 + 链式映射全链路（CASE-LIU 柳智宇跨域验证） |
...
│
├── backend/                           # 后端模块
│   └── wuxing_flowengine/             # 五行知识图谱引擎 (V1.5)
│       ├── config/                    # 流水线配置
...
│       ├── diagnose/                  # 五行诊断模块
│       ├── output/                    # 诊断输出报告 + 验证结果
│       │   ├── reports/               # CASE-LIU 报告 + M3 交付物 + V1.5 交付清单
│       │   └── archive/               # 4 个月度归档（2026-05/06/07/08）
│       └── docs/                      # 设计文档 + 验证任务书 + 实施报告
│
└── deploy/                            # 部署配置
    ├── nginx/
