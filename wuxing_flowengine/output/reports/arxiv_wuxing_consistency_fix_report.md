...
**数据来源**：全部为真实 arXiv API 采集数据（`source_type: "real"`），非模拟/演示数据。6 月 13,364 篇、7 月 10,882 篇、8 月 2,519 篇（8 月截至 9 日，月度未结束）。

**主导路径**：土 → 土 → 火

**关键观察**：
- 6-7 月土主导（ML 基础承载权重最大），8 月火反超（AI 综合 + 多媒体 + 人机交互活跃度上升）——**真实信号，值得关注**
- S_p 稳定在 30-32 区间（克·约束竞争阶段），无虚假漂移

**Fallback 引擎特征标注**（以下四维指标均受 fallback 简化计算影响，真实引擎接入后校准）：

| 指标 | 值 | fallback 成因 | 真实引擎预期 |
|------|-----|-------------|------------|
| E_u | 0.03-0.04 | `1 - H_norm`，11 分类权重均匀 → H_norm ≈ 0.96 → E_u ≈ 0 | 层次化引擎按层间熵均衡度计算，预期更高 |
| O_t | 0.28 | 主导五行占比，11 分类中单一五行最多 3 个 → max ≈ 28% | 分层加权后主导层占比可能更高 |
| C_k | 0.2727 | 同五行分类数 / 总分类数，3/11 ≈ 0.27 | 真实引擎按认知耦合度计算，预期不同 |
| K_y | 1.0 | 分类数密度，11/11 = 1.0（无结构边） | 真实引擎按图密度/相克边计算，预期更低 |

> **S_p = 30.9（"克·约束竞争"）的解读需谨慎**：当前 S_p 由 fallback 简化公式计算，四维指标均偏低或偏高。**S_p 的"克阶段"判定不应被直接解读为 arXiv AI 领域的真实结构状态**——它是 fallback 计算特征与真实数据分布的共同产物。真实引擎接入后 S_p 可能显著变化（预期方向：E_u 和 O_t 上升 → S_p 上升）。
...
### 4.6 数据来源显式标注：`source_type: "real"`

所有树文件 `meta` 中新增 `source_type: "real"` 字段，区分真实采集数据与模拟/演示数据。这影响诊断解读置信度：真实数据中的"火反超"是真实信号，模拟数据中的变化仅是流程验证。未来若引入模拟数据，标注 `source_type: "simulated"`。

### 4.7 Fallback 特征四维全覆盖标注

C_k/K_y 的"fallback 固有特征"标注扩展到 E_u/O_t：四维指标均受 fallback 简化计算影响。S_p 的"克阶段"判定不应被直接解读为领域真实结构状态——它是 fallback 计算特征与真实数据分布的共同产物。真实引擎接入后预期 S_p 上升（E_u 和 O_t 上升）。
