...
|------|------|:---:|:---:|------|------|
| meta-skill.org/studio/ | Cloudflare Pages | ✅ | ✅ | 代理转发 | `_worker.js` (Pages Function) |
| hui-skill.cn/studio/ | Nginx + rsync | ✅ | ✅ | 直连本机 FastAPI | Nginx 反向代理 |
...
`PROJECT_CONTEXT.md` 按分支拆分管理，`master` 为完整记录汇总：

| 分支 | 记录范围 | 同步方向 |
|------|----------|----------|
| `main` | 前端相关任务（页面、样式、部署、Cloudflare Pages 等） | 修改后同步到 `master` |
| `hui-skill-cn` | hui-skill.cn 网站相关任务（后端、服务器、Docker 等） | 修改后同步到 `master` |
| `master` | 完整记录 = `main` + `hui-skill-cn` + 其他开发任务 | 汇总合并来源 |

**同步方式**: 在 `main` 或 `hui-skill-cn` 提交后，`git checkout master && git merge <branch>`，冲突时手动整合。
...
### 9. 八卦图例颜色与实际显示不一致
- **根因**: 图例中八卦区域仅显示纯文字符号，无颜色圆点；3D 节点实际使用 `ring.color`（三层颜色）渲染，不存在八卦维度的颜色体系
- **修复**: 由于八卦信息还在完善中，暂时隐藏八卦图例区域（HTML 注释）
- **文件**: `frontend/studio/template.html`
